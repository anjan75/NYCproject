<?php
  define('ORA_CHARSET_DEFAULT', ''); //WE8MSWIN1252
  ...
  class Database {
    ...
    */
    private function Execute($sql_text, &$bind = false){
        ...
        $stid1 = (int) $stid;
        $this->statements[$stid1]['text'] = $sql_text;
        $this->statements[$stid1]['bind'] = $bind;
        
        if ($bind && is_array($bind)) {
            foreach($bind as $k=>$v){
                oci_bind_by_name($stid, $k, $bind[$k], $this->var_max_size, $this->GetBindingType($bind[$k]));

//oci_bind_by_value($stid, $k, $bind[$k], $this->var_max_size, $this->GetBindingType($bind[$k]));                
            }
        }
        $com_mode = $this->autocommit ? OCI_COMMIT_ON_SUCCESS : OCI_DEFAULT;
        //var_dump($stid);
       
        $this->execute_status = oci_execute($stid, OCI_COMMIT_ON_SUCCESS);
