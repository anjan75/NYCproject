...
class OperatingyardInspection {
	...
	public function CreateInspection($data = null) {
		...
		/*$query = "BEGIN ECR2_PKG.Add_INSPECTIONS(:INSPECTION_TYPE_ID, :TESTING_OFFICER, :RAILROAD_ID, :OBSERVED_EMPLOYEE, :CREW_NUMBER, :TRAIN_NUMBER, :DEPARTMENT_ID, :OCCUPATION_ID, :DESCRIPTION, :PLAN_ID, TO_DATE(:INSPETION_DATE, 'MM/DD/YYYY'), :ENTERED_BY, :CREATION_DATE ); END;"; //'YYYY-MM-DD HH24:MI:SS'
		$this->db->query($query);*/
...
*/		if($this->db->execute()) {
	...
	public function CreateoperatingyardInspectionObservation($data = null) {
		/*echo "<pre>";
		print_r($data);
		echo "<pre>";
		exit();*/
		...
		$this->db->bind(':OBSERVATION_TYPE', $data['observation_type']); //'Direct/Observed'
		$this->db->bind(':MONTHLY_TEST_FLAG', $data['monthly_test_flag']);
		$this->db->bind(':TASK', $data['task']);
		$this->db->bind(':RULES', $data['rules']);
		$this->db->bind(':OUTCOMES', $data['outcomes']); //'Compliant'
		...
		$this->db->bind(':OBS_SPEED_SRC',  $data['obs_speed_src']); //'Radar'
